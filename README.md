...
- [ ] Run `c2rust transpile` on the compilation database.
- [ ] Initialize `crabvpx` Cargo project.
- [ ] Integrate generated `.rs` files into the Cargo structure.

### Phase 3: Assembly FFI Integration (The Shift)
- [ ] Identify necessary Assembly (`.asm`/`.S`) files.
- [ ] Setup `build.rs` to compile and link ASM files using the `cc` crate (and `yasm`/`nasm`).
- [ ] Verify `extern "C"` linkages in the Rust code.
